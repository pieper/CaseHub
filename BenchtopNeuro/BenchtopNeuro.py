...
void main(void) {
  ...
  varyingTextureCoordinate = vec2(coordinate.x, coordinate.y);
...
"""
  ...
  def __init__(self):
    ...
    openGLproperty.SetTexture(vtk.vtkProperty.VTK_TEXTURE_UNIT_1, self.circleTexture())
    openGLproperty.SetPropProgram(self.makeShaderProgram())
    rgba = [0., .7, .7, 1.]

    # TODO: make these uniforms, and then make them vertex attributes
    # populate them from the vtk camera, then attributes from
    # deltas to be used in optimization
    #mat4 perspective (float fovy, float aspect, float near, float far) {
    #mat4 lookAt (vec3 eye, vec3 center, vec3 up) {
    ...
    self.planeMapper.MapDataArrayToVertexAttribute("textureCoordinates", "TCoords", 0, -1)
  ...
  def makeCircleTexture(self):
    ...
    self.textureSource.SetOutValue(50)
  ...
  def makeShaderProgram(self):
    ...
    self.shaderProgram.SetContext(renderWindow)

    self.vertexShader=vtk.vtkShader2()
    self.vertexShader.SetType(vtk.VTK_SHADER_TYPE_VERTEX)
    self.vertexShader.SetSourceCode(vertexShaderSource)
    ...
    self.fragmentShader.SetSourceCode(fragmentShaderSource)
